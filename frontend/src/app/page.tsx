...
What are we shopping for today?`,
  ...
  const [hamperTitle, setHamperTitle] = useState<string>('Shopping Cart & Hamper Builder');
  ...
  const handleAddToHamper = (product: Product) => {
    setCart((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, price: product.price, quantity: 1, image: product.image, url: product.url }];
    });
  ...
  const handleClearCart = () => {
    ...
    setHamperTitle('Shopping Cart & Hamper Builder');
  };
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      // Call API chat orchestration endpoint with conversation history and current form state
      ...
      if (intent.hamper_title) setHamperTitle(intent.hamper_title);
